...
class AccountRepository:
        ...
        with db_transaction() as conn:
            conn.execute(
                ...
                INSERT INTO accounts (
                    ...
                    min_balance, pin_hash, account_status, created_at, version
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
    ...
    def count(self, account_status: Optional[AccountStatus] = None) -> int:
        """
        ...
        try:
            if account_status is not None:
                row = conn.execute(
                    "SELECT COUNT(*) AS cnt FROM accounts WHERE account_status = ?", (account_status.value,)
                ).fetchone()
            else:
    ...
    def update_status(self, account_id: UUID, account_status: AccountStatus) -> Account:
        """
        Updates the account_status of an account within the SQL database.
        ...
        with db_transaction() as conn:
            cursor = conn.execute(
                "UPDATE accounts SET account_status = ?, version = version + 1 WHERE account_id = ?",
                (account_status.value, str(account_id)),
            )
            if cursor.rowcount == 0:
        ...
        with db_transaction() as conn:
            cursor = conn.execute(
                ...
                UPDATE accounts SET
                    ...
                    pin_hash = ?, account_status = ?, version = version + 1
                WHERE account_id = ? AND version = ?
                """,
                (
                    account.account_number,
